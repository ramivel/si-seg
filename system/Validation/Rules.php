...
class Rules
{
     ...
     *
     * Example:
     *    is_unique_with_schemas[schema.table.field,ignore_field,ignore_value]
     *    is_unique_with_schemas[public.users.email,id,5]
     *
     * @param string $str
     */
    public function is_unique_with_schemas(?string $str, string $field, array $data): bool
    {
        // Grab any data for exclusion of a single row.
        [$field, $ignoreField, $ignoreValue] = array_pad(explode(',', $field), 3, null);
     ...
     * @param string $str
     */
    public function verificar_hr($str = null): bool
    {
        $db = Database::connect('sincobol');
        $row = $db->table('contratos_licencias.solicitud_licencia_contrato')
            ->select('1')
            ->where('fk_acto_administrativo > 0 AND id = '.$str)
            ->limit(1);
        return $row->get()->getRow() === null;
    }

    public function verificar_hr_cmn_cmc($str = null): bool
    {
        $db = Database::connect();
        $row = $db->table('public.acto_administrativo')
            ->select('1')
            ->where('deleted_at is null AND fk_hoja_ruta = '.$str)
            ->limit(1);
        return $row->get()->getRow() === null;
    }

    public function verificar_area_minera_cmn_cmc($str = null): bool
    {
        $db = Database::connect();
        $row = $db->table('public.acto_administrativo')
            ->select('1')
            ->where('deleted_at is null AND fk_area_minera = '.$str)
            ->limit(1);
        return $row->get()->getRow() === null;
    }
}
